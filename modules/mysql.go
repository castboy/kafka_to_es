...
func xdrSql(x BackendObj, id int64, t string) string {
	...
	sql = fmt.Sprintf(`insert into %s (%s) values (
		...
		x.Http.Host, x.Http.Url, x.Http.XonlineHost, x.Http.UserAgent, x.Http.ContentType,
		...
		x.Http.Version, x.Http.HeadFlag, x.Http.ServFlag, boolToInt(x.Http.RequestFlag), x.Http.Browser, x.Http.Portal,
